...
class RaytraceRender(Rasterizer):
    ...
    def forward__(
        ...
        **kwargs
        ...
        # colorimg=color.detach().reshape(height,width,3).cpu().numpy()
        # colorimg=Image.fromarray((colorimg*255).astype(np.uint8))
        # colorimg.save("color.png")

        # def saveimg(img,path):
            
        #     img=img.detach().reshape(height,width,3).cpu().numpy()
        #     img=Image.fromarray((img*255).astype(np.uint8))
        #     img.save(path)
        
        # normal_controlnet=self.compute_controlnet_normals(normals,w2c,height,width)
        # saveimg(normal_controlnet,"normal_control.png")
        # saveimg((normals.reshape(batch_size,height,width,3)+1.0)/2,"normal_raw.png")
        # import ipdb
        # ipdb.set_trace()
        ...
        # out = {"opacity": mask_aa, "mesh": self.mesh}
        ...
        #     gb_normal_aa, rast, v_pos_clip, self.mesh.t_pos_idx
        # )
        ...
        #             self.mesh.v_tng, rast, self.mesh.t_pos_idx
        #         )
        ...
        #     gb_rgb_aa = self.ctx.antialias(gb_rgb, rast, v_pos_clip, self.mesh.t_pos_idx)
