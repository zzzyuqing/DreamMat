    ...
    def forward(
        ...
        **kwargs
        ...
        if render_rgb:
            ...
            rgb_fg=outputs['color']
            gb_rgb_fg = torch.zeros(batch_size, height, width, 3).to(rgb_fg)
            albedo = torch.zeros(batch_size, height, width, 3).to(self.device)
            roughness = torch.zeros(batch_size, height, width, 3).to(self.device)
            metalness = torch.zeros(batch_size, height, width, 3).to(self.device)
            gb_rgb_fg[selector] = rgb_fg
            albedo[selector]=outputs['albedo']
            roughness[selector]=outputs['roughness']
            metalness[selector]=outputs['metalness']

            gb_rgb_bg = self.background(dirs=gb_viewdirs)
            gb_rgb = torch.lerp(gb_rgb_bg, gb_rgb_fg, mask.float())
            gb_rgb_aa = self.ctx.antialias(gb_rgb, rast, v_pos_clip, mesh.t_pos_idx)

            out.update({"comp_rgb": gb_rgb_aa, "comp_rgb_bg": gb_rgb_bg, "albedo":albedo,'roughness':roughness,"metalness":metalness})
